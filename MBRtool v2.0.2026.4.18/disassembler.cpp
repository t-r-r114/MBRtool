...
const char* disassemble_instruction(uint64_t base_addr, const uint8_t *code, size_t *size) {
    ...
    // 动态修正 16/32 位模式的寄存器和操作数大小
    int current_operand_size = info->operand_size;
    std::string current_mnemonic = info->mnemonic ? info->mnemonic : "";

    if (DisassemblermodePtr->g_disasm_mode == 16) {
        // 1. 修正操作数大小：32位下的 4字节立即数，在16位模式下默认是 2字节
        if (current_operand_size == 4) {
            current_operand_size = 2;
        }

        // 2. 修正助记符：将硬编码的 32位通用寄存器名 降级为 16位
        const char* reg32[] = {"EAX", "ECX", "EDX", "EBX", "ESP", "EBP", "ESI", "EDI"};
        const char* reg16[] = {"AX",  "CX",  "DX",  "BX",  "SP",  "BP",  "SI",  "DI"};

        for (int i = 0; i < 8; ++i) {
            size_t pos = current_mnemonic.find(reg32[i]);
            if (pos != std::string::npos) {
                current_mnemonic.replace(pos, 3, reg16[i]);
            }
        }
    }

    // 获取修正后的助记符指针
    const char *mnemonic = current_mnemonic.c_str();
    // ==========================================

    // 处理带立即数的指令 (注意：这里把原来的 info->operand_size 全换成了 current_operand_size)
    if (current_operand_size > 0) {
        uint32_t imm = 0;
        // 小端模式读取立即数
        for (int i = 0; i < current_operand_size; i++) {
            imm |= (static_cast<uint32_t>(code[offset + i]) << (i * 8));
        }
        offset += current_operand_size;
        ...
        if (strstr(mnemonic, "J") == mnemonic || strstr(mnemonic, "CALL") == mnemonic) {
            ...
            // 根据修正后的大小计算跳转
            if (current_operand_size == 1) {
                int8_t rel8 = static_cast<int8_t>(imm);
                target = current_end + rel8;
            } else if (current_operand_size == 2) { // 增加 2 字节跳转处理 (16位模式)
                int16_t rel16 = static_cast<int16_t>(imm);
                target = current_end + rel16;
            } else if (current_operand_size == 4) {
            ...
            } else if (current_operand_size == 2) {  // 增加对 2 字节立即数的格式化
