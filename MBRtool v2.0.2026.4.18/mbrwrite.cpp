...
int MBRwrite::mBRWRITE(int diskNumber) {
    // 1. 确认逻辑（可以在提示框中加入磁盘号显示）
    QString warnMsg = QString("即将向 PhysicalDrive%1 写入 MBR，此操作危险！\n是否继续？").arg(diskNumber);
    int reply = MessageBoxW(NULL, (LPCWSTR)warnMsg.utf16(), L"警告", MB_YESNO | MB_ICONWARNING);

    if (reply != IDYES) return 1;
